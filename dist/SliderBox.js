...
import { View, Image, ActivityIndicator, TouchableHighlight, Dimensions } from 'react-native';

import Carousel, { Pagination } from 'react-native-snap-carousel-deprecated-prop-types'; //Thank From distributer(s) of this lib
...
export class SliderBox extends Component {
	...
	get pagination() {
		...
		const { images, dotStyle, dotColor, inactiveDotColor, paginationBoxStyle, paginationBoxVerticalPadding } = this.props;
		return (
			<Pagination
				borderRadius={2}
	...
	render() {
		const { images, circleLoop, autoplay, parentWidth, loopClonesPerSide, autoplayDelay, useScrollView, autoplayInterval } = this.props;
